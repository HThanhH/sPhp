    ...
    <div class="main-content">
            ...
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price']) ) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                            $result = mysqli_query($con, "UPDATE `product` SET `name` = '" . $_POST['name'] . "',`image` =  '" . $image . "',`id_loai` =  '" . $id_l . "',`L` =  '" . $L . "', `price` = " . str_replace('.', '', $_POST['price']) . ", `content` = '" . $_POST['content'] . "', `last_updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { //Thêm sản phẩm
                            $result = mysqli_query($con, "INSERT INTO `product` (`id`, `name`, `image`, `id_loai`,`L`, `price`, `content`, `created_time`, `last_updated`) VALUES (NULL, '" . $_POST['name'] . "','" . $image . "','" . $id_l . "','" . $L . "' ," . str_replace('.', '', $_POST['price']) . ", '" . $_POST['content'] . "', " . time() . ", " . time() . ");");
                        }
                ...
                <form id="editing-form" method="POST" action="<?= (!empty($product) && !isset($_GET['task'])) ? "?action=edit&id=" . $_GET['id'] : "?action=add" ?>"  enctype="multipart/form-data">
                    ...
                    <div class="wrap-field">
                        ...
                        <input style="text-transform: uppercase; " type="text" name="name" value="<?= (!empty($product) ? $product['name'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Giá sản phẩm: </label>
                        <input style="text-transform: uppercase; " type="text" name="price" value="<?= (!empty($product) ? number_format($product['price'], 0, ",", ".") : "") ?>" />
                        <label>đ </label>
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Thể tích: </label>
                        <input style="text-transform: uppercase; " type="text" name="L" value="<?= (!empty($product) ? number_format($product['L'], 0, ",", ".") : "") ?>" />
                        <label>L </label>
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
        ...
        <?php } ?>
                    ...
                    <div class="wrap-field">
                        ...
                        <select name="id_loai" id="id_loai">
                            <option >Lựa chọn</option>
                            <?php 
                            define('HOST', 'localhost');
                            define('USERNAME', 'root');
                            define('PASSWORD', '');
                            define('DATABASE', 'demo_db');
						  ...
						  {
							  if($item['id_loai']==$product['id_loai']){
								echo '<option selected value = "'.$item['id_loai'].'">'.$item['ten_loai'].'</option>';
							  } else{
								echo '<option value = "'.$item['id_loai'].'">'.$item['ten_loai'].'</option>';
							  }
						  }
						  ?>
                        </select>
