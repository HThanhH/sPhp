    ...
    <body>
       ...
       <?php
                ...
                case "submit":
                    if (isset($_POST['update_click'])) { //Cập nhật số lượng sản phẩm
                        ...
                        if ($error == false && !empty($_POST['quantity'])) { //Xử lý lưu giỏ hàng vào db
                            $products = mysqli_query($con, "SELECT * FROM `sanpham` WHERE `id_sp` IN (" . implode(",", array_keys($_POST['quantity'])) . ")");
                            $total = 0;
                            $orderProducts = array();
                            ...
                            $insertOrder = mysqli_query($con, "INSERT INTO `orders` (`id`, `name`, `phone`, `address`, `note`, `total`, `created_time`, `last_updated`) VALUES (NULL, '" . $_POST['name'] . "', '" . $_POST['phone'] . "', '" . $_POST['address'] . "', '" . $_POST['note'] . "', '" . $total . "',  NOW()  , '" . time() . "');");
                            $orderID = $con->insert_id;
                            $insertString = "";
                            foreach ($orderProducts as $key => $product) {
                                $insertString .= "(NULL, '" . $orderID . "', '" . $product['id_sp'] . "', '" . $_POST['quantity'][$product['id_sp']] . "', '" . $product['gia'] . "', '" . time() . "', '" . time() . "')";
                                if ($key != count($orderProducts) - 1) {
                                    $insertString .= ",";
                                }
                            }
                            $insertOrder = mysqli_query($con, "INSERT INTO `order_detail` (`id`, `order_id`, `id_sp`, `quantity`, `price`, `created_time`, `last_updated`) VALUES " . $insertString . ";");
        ...
        if (!empty($_SESSION["cart"])) {
            $products = mysqli_query($con, "SELECT * FROM `sanpham` WHERE `id_sp` IN (" . implode(",", array_keys($_SESSION["cart"])) . ")");
        }
                        ...
                        if (!empty($products)) {
                            ...
                            while ($row = mysqli_fetch_array($products)) {
                                ...
                                <tr>
                                    ...
                                    <td class="product-quantity"><input type="text" value="<?= $_SESSION["cart"][$row['id_sp']] ?>" name="quantity[<?= $row['id_sp'] ?>]" /></td>
                                    <td class="total-money"><?= number_format($row['gia'] * $_SESSION["cart"][$row['id_sp']], 0, ",", ".") ?></td>
                                    <td class="product-delete"><a href="cart.php?action=delete&id=<?= $row['id_sp'] ?>">Xóa</a></td>
                                </tr>
                                <?php
